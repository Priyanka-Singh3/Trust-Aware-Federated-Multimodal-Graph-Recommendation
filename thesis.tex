...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{correct_archi.jpeg}
...
\begin{itemize}
    \item \textbf{User-Item Matrix}: Interactions are stored in a sparse interaction matrix $R \in \mathbb{R}^{m \times n}$, where $m$ and $n$ denote the number of users and items. With 10,400 users and 1,694 items, this matrix contains 17,617,600 possible entries but only 41,493 observed interactions (99.76\% sparsity).
...
\begin{itemize}
    \item $U$: set of 10,400 user nodes
    \item $I$: set of 1,694 item nodes
    \item $E$: set of 41,493 observed interaction edges (ratings)
\end{itemize}
...
\textbf{Architecture:} 2 GNN layers ($L=2$), 256-d embeddings, SAGE convolution, 50 epochs, BPR loss.
...
\begin{equation}
\theta^{t+1} = \sum_{k=1}^{K} \frac{\tau_k}{\sum_{j=1}^K \tau_j} \theta_k^{t+1}
\end{equation}

where $\tau_k \in [0,1]$ is the composite trust score for client $k$. This score inherently accounts for the client's dataset size via a performance bonus, rendering the standard dataset size multiplier $n_k$ redundant. This weighting scheme reduces the influence of potentially malicious or low-quality updates, improving model robustness compared to standard FedAvg where all clients contribute equally ($\tau_k = 1$ for all $k$).

\subsection{Communication Efficiency}
Only model parameters (2.38M floats $\approx$ 9.52 MB) are transmitted, never raw user data. For $T=20$ rounds, total communication per client is 380.8 MB (upload + download), enabling practical deployment on standard network connections.
...
\begin{itemize}
    \item \textbf{Optimizer:} Adam with learning rate $10^{-3}$, weight decay $10^{-5}$
    \item \textbf{Learning Rate Schedule:} CosineAnnealingLR over 50 epochs
    \item \textbf{Batch Size:} 1024 training pairs (1 positive + 1 negative per user)
    \item \textbf{Gradient Clipping:} Max norm 1.0 to prevent exploding gradients
    \item \textbf{Regularization:} Dropout 0.1 on all hidden layers
...
\begin{equation}
\theta_{g} = \sum_{k=1}^{K} w_k \theta_k, \quad \text{where } w_k = \frac{\tau_k}{\sum_{j=1}^K \tau_j}
\end{equation}
where $\tau_k$ is the trust score for client $k$. Trust scores are computed based on update quality, historical reliability, gradient similarity to the global consensus, and dataset size.
...
Initialization & Server $\to$ Clients & 9.52 MB & Broadcast initial global model $\theta^{(0)}$ \\ \hline
Local Training & Local & -- & Each client trains for $E$ epochs \\ \hline
Update Upload & Client $\to$ Server & 9.52 MB & Upload model weights with DP noise \\ \hline
Aggregation & Server & -- & Trust-weighted averaging of client updates \\ \hline
Distribution & Server $\to$ Clients & 9.52 MB & Broadcast updated global model \\ \hline
\end{tabular}
\end{table}

\textit{Note:} Total communication per client: $2 \times T \times 9.52$ MB = 380.8 MB for $T=20$ rounds. FP16 compression can reduce this by 50\%.
...
Total Users & 10,400 & Unique reviewers \\ \hline
Total Items (Businesses) & 1,694 & Restaurants and services \\ \hline
Total Interactions & 41,493 & User-business reviews \\ \hline
Interactions per User (mean) & 4.0 & Cold-start scenario \\ \hline
Interactions per Item (mean) & 24.5 & Long-tail distribution \\ \hline
Sparsity Ratio & 99.76\% & $\frac{|E|}{|U|\times|I|}$ \\ \hline
...
Client 0 & 2,080 & 8,300 & 3.99 \\ \hline
Client 1 & 2,080 & 8,298 & 3.99 \\ \hline
Client 2 & 2,080 & 8,295 & 3.99 \\ \hline
Client 3 & 2,080 & 8,305 & 3.99 \\ \hline
Client 4 & 2,080 & 8,295 & 3.99 \\ \hline
\textbf{Total} & \textbf{10,400} & \textbf{41,493} & \textbf{3.99} \\ \hline
\end{tabular}
\end{table}

\textit{Note:} 10,400 interactions reserved for testing (leave-one-out protocol).

\subsection{Evaluation Protocol}
A rigorous \textbf{leave-one-out} evaluation protocol ensures fair assessment:

\begin{itemize}
    \item \textbf{Training Set:} 31,093 interactions
    \item \textbf{Test Set:} Highest rated interaction per user (10,400 test users)
...
The T-FedMMG framework was evaluated on the Yelp dataset using the leave-one-out protocol (10,400 test users, 1 positive + 99 negatives each). As shown in Table~\ref{tab:baselines}, our trained model achieves HR@10 of 0.2473 and NDCG@10 of 0.1271. The model demonstrates a 2.47$\times$ improvement over the random baseline and consistently beats the popularity-based approach.
\begin{table}[h]
\centering
\small
...
\textbf{T-FedMMG (Enhanced)} & \textbf{0.2473} & \textbf{0.1271} & \textbf{2.47$\times$} & \textbf{FL + Trust + Multimodal Graph} \\ \hline
\end{tabular}
\end{table}

\textit{Note:} HR = Hit Rate (recall at K). Results from leave-one-out evaluation on 10,400 test users.

\begin{figure}[htbp]
\centering
\includegraphics[width=1\textwidth]{fig1_baseline_comparison.png}
\caption[Baseline Comparison]{Baseline comparison: T-FedMMG achieves HR@10 of 0.2473, 2.47$\times$ better than random baseline. The GNN architecture with attention mechanisms demonstrates superior performance while maintaining the core Trust-Aware Federated Multimodal Graph Recommendation methodology.}
\label{fig:baselines}
\end{figure}
...
Hit Rate (HR) & 0.1510 & 0.2473 & 0.4045 & 2.47$\times$ over random \\ \hline
NDCG & 0.0963 & 0.1271 & 0.1667 & 2.73$\times$ over random \\ \hline
Catalog Coverage & -- & 11.87\% & -- & Good diversity \\ \hline
\end{tabular}
\end{table}
...
\textbf{T-FedMMG (Enhanced)} & \textbf{0.2473} & \textbf{0.1271} & \textbf{11.87\%} & \textbf{2.47$\times$} \\ \hline
\end{tabular}
\end{table}

\textit{Key Finding:} T-FedMMG achieves a massive 2.47$\times$ higher HR@10 and 2.73$\times$ higher NDCG@10 compared to the random baseline, demonstrating that attention mechanisms and adaptive optimization significantly improve graph-based recommendation performance.

\textit{Note:} All configurations use 256-d embeddings and 50 epochs. Full model uses gated fusion mechanism instead of simple concatenation.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{fig5_coverage_stats.png}
\caption[Catalog Coverage and Diversity Statistics]{Catalog coverage and diversity statistics: T-FedMMG achieves 11.87\% coverage, demonstrating significant catalog exploration capabilities.}
\label{fig:coverage}
\end{figure}

\section{Cold-Start Analysis}
Figure~\ref{fig:coldstart} shows the enhanced model's performance for users with different numbers of training interactions. With an average of only 4.0 interactions per user, the system relies heavily on multimodal features and attention mechanisms to substitute for missing historical data. Users with 5+ interactions achieve the highest HR@10 (0.38), while cold-start users (1-2 interactions) still maintain performance above random baseline, demonstrating the effectiveness of our enhanced T-FedMMG approach.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{fig3_cold_start.png}
\caption[Cold-Start Performance]{Cold-start performance under sparse interaction settings: HR@10 by number of user interactions. T-FedMMG shows improved performance across all interaction levels with users having 5+ interactions achieving HR@10 of 0.38.}
\label{fig:coldstart}
\end{figure}

\section{Training Convergence Analysis}
Figure~\ref{fig:convergence} illustrates the T-FedMMG training convergence behavior. The model achieves solid convergence with BPR loss dropping to 0.3262 over 50 epochs, demonstrating the effectiveness of adaptive learning rate scheduling and attention mechanisms while avoiding catastrophic overfitting on the sparse graph. This represents a highly stable convergence pattern.
...
Accuracy (HR@10) & 0.2473 & $>$0.20 & Excellent \\ \hline
NDCG@10 & 0.1271 & $>$0.10 & Excellent \\ \hline
Catalog Coverage & 11.87\% & $>$10\% & Good \\ \hline
Training Convergence & 50 epochs & 50+ & Perfect \\ \hline
...
\caption[T-FedMMG Training Convergence]{T-FedMMG training convergence: BPR loss decreases from initial values to a stable convergence point (0.3262) over 50 epochs, demonstrating effective optimization with adaptive learning rate scheduling without overfitting.}
...
\section{Comparison: Proof-of-Concept vs. Scaled Dataset}
To validate the robustness of the T-FedMMG framework, we scaled the dataset from an initial Proof-of-Concept (PoC) level (827 users, 2.7k interactions) to a dense scale (10,400 users, 41.5k interactions). 

The results demonstrated immense improvements in model behavior:
\begin{enumerate}
    \item \textbf{Improved Generalization:} On the PoC dataset, the BPR loss dropped to 0.0000 over 200 epochs, indicating catastrophic overfitting due to the extreme sparsity. On the scaled dataset, BPR loss stabilized smoothly at 0.3262 over 50 epochs, proving the model was actively learning generalized patterns.
    \item \textbf{Accuracy Leap:} The absolute HR@10 jumped from 0.1848 to 0.2473, a remarkable 33.8\% relative improvement.
    \item \textbf{Diversity and Coverage:} With more visual and textual features correctly processed across 1,694 businesses, the Catalog Coverage almost doubled from 6.97\% to 11.87\%, confirming that the model effectively explores the "long tail" of the dataset instead of collapsing into a popularity bias.
\end{enumerate}

Figure~\ref{fig:dataset_comparison} and Figure~\ref{fig:coverage_comparison} explicitly illustrate the performance leap achieved by evaluating T-FedMMG on the scaled dataset.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{fig5_dataset_scaling_comparison.png}
\caption[Dataset Scaling Comparison]{Dataset Scaling Comparison: Scaling the dataset to 41k interactions resulted in a 33.8\% relative improvement in HR@10.}
\label{fig:dataset_comparison}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{fig6_catalog_coverage_comparison.png}
\caption[Catalog Coverage Improvement]{Catalog Coverage Improvement: The scaled dataset nearly doubled the catalog coverage, showing significantly better utilization of multimodal features.}
...
\begin{itemize}
    \item \textbf{Trained Enhanced GNN Model:} Successfully trained an \textbf{Enhanced T-FedMMG} model with 2.38M parameters on Yelp dataset (10,400 users, 1,694 items, 41,493 interactions). The model achieves \textbf{HR@10 of 0.2473} (2.47$\times$ better than random baseline) and \textbf{NDCG@10 of 0.1271}.
    \item \textbf{Hyperparameter Optimization:} Trained for \textbf{50 epochs} with \textbf{256-dimensional embeddings}, achieving \textbf{solid convergence} with BPR loss dropping to 0.3262. Optimized learning rate ($10^{-3}$) and dropout (0.1) with adaptive scheduling for sparse data.
    \item \textbf{Privacy-Preserving Architecture:} Implemented federated learning with 5 clients ($\sim$8,300 interactions each), ensuring raw data never leaves local devices. Only model gradients are shared with the global server.
    \item \textbf{System Deployment:} Implemented FastAPI-based inference API with 11.87\% catalog coverage. The system architecture supports real-time recommendations with efficient model serving capabilities (see Chapter 5 for communication overhead analysis).
    \item \textbf{Comprehensive Evaluation:} Performed leave-one-out evaluation on 10,400 test users with 1 positive + 99 negatives protocol. Enhanced model demonstrates consistent performance: HR@5 (0.1510), HR@10 (0.2473), HR@20 (0.4045).
\end{itemize}
...
\textbf{Critical Insight:} T-FedMMG achieves a massive 2.47$\times$ higher HR@10 and 2.73$\times$ higher NDCG@10 compared to the random baseline, demonstrating that attention mechanisms and adaptive optimization significantly improve graph-based recommendation performance.

\section{Limitations}
Despite the promising results, certain limitations were identified during the study:
\begin{enumerate}
    \item \textbf{Extreme Data Sparsity:} Only 41,493 interactions across 10,400 users (4.0 per user) presents limited graph connectivity. Industrial-scale datasets with millions of interactions would significantly improve results.
    \item \textbf{Modest Absolute Metrics:} While achieving 2.47$\times$ improvement over random, HR@10 of 0.2473 indicates significant room for improvement compared to state-of-the-art systems trained on dense datasets.
    ...
    \item \textbf{Computational Overhead:} Training for 50 epochs with 256-d embeddings requires significant compute resources, and trust computation adds inference latency overhead.
\end{enumerate}
...
This research emphasizes the necessity of integrating trust and privacy into the next generation of recommendation systems. T-FedMMG demonstrates that it is possible to achieve meaningful recommendation performance (2.47$\times$ better than random) with strong diversity (11.87\% coverage) without compromising user data, even under extreme data sparsity (99.76\%). By balancing the technical requirements of Multimodal Learning and Federated Privacy, this work contributes a practical framework for privacy-preserving recommendation in data-scarce environments.
%---------------------------------
...
\midrule
Upload (Client $\rightarrow$ Server) & 1  & 9.52 MB & None (Baseline) \\
Upload (Client $\rightarrow$ Server) & 5  & 9.52 MB & None \\
Upload (Client $\rightarrow$ Server) & 10 & 9.52 MB & None \\
Upload (Client $\rightarrow$ Server) & 20 & 9.52 MB & None \\
\midrule
Download (Server $\rightarrow$ Client) & 1  & 9.52 MB & None \\
Download (Server $\rightarrow$ Client) & 5  & 9.52 MB & None \\
Download (Server $\rightarrow$ Client) & 10 & 9.52 MB & None \\
Download (Server $\rightarrow$ Client) & 20 & 9.52 MB & None \\
\midrule
\textbf{Total per Client (20 rounds)} & -- & \textbf{380.8 MB} & -- \\
\textbf{Total all Clients (5 clients)} & -- & \textbf{1,904 MB} & -- \\
\bottomrule
\end{tabular}
\end{table}

\vspace{0.5em}
\textit{Note:} Model size = 2.38M parameters $\times$ 4 bytes (float32) $\approx$ 9.52 MB per round. FP16 compression could reduce this by 50\% to 4.76 MB per round.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Configuration} & \textbf{Text} & \textbf{Image} & \textbf{HR@10} & \textbf{NDCG@10} \\
\midrule
Random Baseline              & --           & --           & 0.1000 & 0.0465 \\
Text-Only (TF-IDF)           & $\checkmark$ & $\times$     & 0.1982 & 0.1032 \\
Image-Only (ResNet-18)       & $\times$     & $\checkmark$ & 0.1607 & 0.0837 \\
Text + Image (Concatenation) & $\checkmark$ & $\checkmark$ & 0.2137 & 0.1143 \\
\midrule
\textbf{T-FedMMG (Full)}     & $\checkmark$ & $\checkmark$ & \textbf{0.2473} & \textbf{0.1271} \\
\bottomrule
\end{tabular}
\end{table}
...
Batch Size          & 1024      & [256, 1024]            & Number of training pairs per batch \\
...
Weight Decay        & $10^{-5}$ & [$10^{-6}$, $10^{-4}$] & L2 regularization parameter \\
...
Text Projection (x2)   & Linear(1000$\to$128) for User and Item                  & 128-d   & 256,000 trainable \\
Image Projection (x2)  & Linear(512$\to$128) for User and Item                   & 128-d   & 131,072 trainable \\
Multimodal Attention   & MultiheadAttention (8 heads)                            & 256-d   & 263,168 trainable \\
\midrule
\textbf{Fusion Layer}  & Concat[embedding, multimodal]                           & 512-d   & 0 trainable \\
\bottomrule
\end{tabular}}
\end{table}

\vspace{0.5em}
\textit{Note:} Total trainable parameters $\approx$ 2.38M. ResNet-18 backbone is frozen; only projection, attention, and GNN layers are trained.
...
\midrule
User Embedding         & nn.Embedding                            & 10,400   & 256 & -- \\
Item ID Embedding      & nn.Embedding                            & 1,694   & 256 & -- \\
\midrule
User Text Projection   & Linear                                  & 1,000 & 128 & -- \\
User Image Projection  & Linear                                  & 512   & 128 & -- \\
Item Text Projection   & Linear                                  & 1,000 & 128 & -- \\
Item Image Projection  & Linear                                  & 512   & 128 & -- \\
\midrule
User/Item Attention    & MultiheadAttention (8 heads)            & 256   & 256 & Softmax \\
\midrule
GNN Layer 1            & GATConv (4 heads)                       & 512   & 256 & -- \\
GNN Layers 2-3         & GATConv (4 heads)                       & 256   & 256 & -- \\
\midrule
Residual Layers        & Linear (x2)                             & 256   & 256 & -- \\
Layer Normalization    & LayerNorm (x3)                          & 256   & 256 & -- \\
...
\vspace{0.5em}
\textit{Note:} The input to the GNN layers (512-d) is the concatenation of the 256-d node embedding and the 256-d attention-fused multimodal features.
...
\midrule
GNN Layer 1 (Input)      & GATConv (4 heads)                     & 512 (Emb+MM) & 256 & -- \\
GNN Layer 2              & GATConv (4 heads) + Residual          & 256   & 256 & -- \\
GNN Layer 3              & GATConv (4 heads) + Residual          & 256   & 256 & -- \\
\midrule
Output Projection        & Linear + Dropout(0.1)                 & 512   & 256 & --   \\
\bottomrule
\end{tabular}}
...
\FOR{each communication round $t = 1, \ldots, T$}
    ...
    \FOR{each client $k$}
        \STATE $q_k \leftarrow \text{CosineSimilarity}(\theta_k^{(t,E)} - \theta^{(t-1)},\ \bar{\Delta})$ \COMMENT{Update similarity}
        \STATE $\tau_k \leftarrow \text{ComputeTrust}(k, q_k)$ \COMMENT{Includes consistency, performance, and $q_k$}
    \ENDFOR
    \STATE $\theta^{(t)} \leftarrow \sum_{k} \frac{\tau_k}{\sum_j \tau_j} \cdot \theta_k^{(t,E)}$ \COMMENT{Trust-weighted FedAvg}
...
\midrule
Total Users                    & 10,400 \\
Total Businesses (Items)       & 1,694 \\
Total Interactions (Reviews)   & 41,493 \\
Average Interactions per User  & 4.0 \\
Average Interactions per Business & 24.5 \\
Sparsity Ratio                 & 99.76\% \\
